...
{
    public class ExportNHCInterestCamp : IRequest<Response<string>>
    {
    ...
    public class ExportNHCInterestCampHandler : IRequestHandler<ExportNHCInterestCamp, Response<string>>
    {
        ...
        public ExportNHCInterestCampHandler(
        ...
        public async Task<Response<string>> Handle(ExportNHCInterestCamp request, CancellationToken cancellationToken)
        {
            //new
            ...
            try
            {
                ...
                var maxIdPerAddress = _context.nhc_interest_camp
    .GroupBy(report => new { TruncatedNumber = report.callernumber.Substring(Math.Max(0, report.callernumber.Length - 9)), report.UCID })
    .Select(grouped => new
    {
        TruncatedNumber = grouped.Key.TruncatedNumber,
        UCID = grouped.Key.UCID,
        MaxId = grouped.Max(x => x.id)
    });

               var query = _context.Pim_contact_attempts_historyLog.Where(x=>x.Contact_attempt_time >=startDate && x.Contact_attempt_time<=endDate)
    .Select(nhc => new
    {
        nhc,
        maxCan = maxIdPerAddress.FirstOrDefault(maxCan => maxCan.TruncatedNumber == nhc.Address && maxCan.UCID == nhc.Ucid)
    })
    .Select(joined => new
    {
        joined.nhc,
        can = _context.nhc_interest_camp.FirstOrDefault(can => can.id == (joined.maxCan != null ? joined.maxCan.MaxId : (int?)null))
    })
    .Select(joined => new
    {
        Id = joined.can!.id ,
        Address = joined.nhc.Address,
        //  Month = joined.nhc.Contact_attempt_time !=null ? joined.nhc.Contact_attempt_time.Value.AddHours(8).ToString("yyyy-MM-dd") : string.Empty,
        Month=  joined.nhc.Contact_attempt_time!.Value.AddHours(8).ToString("yyyy-MM-dd") ,

        UCID = joined.nhc.Ucid,
        VisitedNote = joined.can!.Visited_Note,
        SubmitDate = joined.can!.submitdate,
        Status = GetStatus(joined.nhc.Completion_Code_id) 
                ...
                if(startDate.Date == endDate.Date) { excelName = $"NHCInterest-{startDate.ToString("yyyyMMdd")}.xlsx"; }
                ...
                using (ExcelPackage package = new ExcelPackage(file))
                {
                    ...
                    worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "id"; j++;
                    ...
                    worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "ucid"; j++;
                    //worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "اسم رمز المكالمة"; j++;
                    worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "ملاحظة الزيارة"; j++;
                    worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "التوقيت"; j++;
                    worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "الحالة"; j++;
                   // worksheet.Column(j).Width = 20; worksheet.Cells[1, j].Value = "المحاولات"; j++;
                    ...
                    foreach (var item in query)
                    {
                        ...
                        //if (item.lastIte =1;m.Contact_attempt_time >= ChangingTime) { extra = 8; }
                        int columnNumber = 1;
                         worksheet.Cells[rowNumber, columnNumber].Value = item.Id ; columnNumber++;
                        //  worksheet.Cells[rowNumber, columnNumber].Value = (item.Contact != null) ? item.Contact?.PersonalInfo?.FullNameAr : ""; columnNumber++;
                        worksheet.Cells[rowNumber, columnNumber].Value = item.Address; columnNumber++;
                        worksheet.Cells[rowNumber, columnNumber].Value = item.Month; columnNumber++;
                        ...
                        worksheet.Cells[rowNumber, columnNumber].Value = item.UCID; columnNumber++;
                        ...
                        worksheet.Cells[rowNumber, columnNumber].Value = item.VisitedNote; columnNumber++;
                        worksheet.Cells[rowNumber, columnNumber].Value = item.SubmitDate; columnNumber++;
                        worksheet.Cells[rowNumber, columnNumber].Value = item.Status; columnNumber++;
        ...
        static string GetStatus(int completionCodeId)
        {
            return completionCodeId switch
            {
                12 or 13 => "تم الرد",
                55 or 45 or 69 or 18 or 9 or 8 or 5 or 15 or 17 or 7 or 54 or 43 or 4 => "لم يتم الرد",
                16 => "لم يتم الوصول",
                10 => "الرقم مشغول",
                11 => "رنين و لم يتم الرد",
                14 => "رد آلي",
                _ => "غير معروف"
            };
        }
